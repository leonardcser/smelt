...
/// `anthropic-compatible` provider using compact SSE (`data:{...}` without a
/// space). Exercises both the provider type and the SSE parser's whitespace
/// tolerance in one shot.
#[tokio::test]
async fn anthropic_compatible_sse() {
    let h = Harness::new().await;
    h.write_config("anthropic-compatible", "kimi-test");
    h.write_init_lua("");
    h.mount_anthropic_compatible_sse(&[
        serde_json::json!({
            "type": "message_start",
            "message": {
                "id": "msg_test",
