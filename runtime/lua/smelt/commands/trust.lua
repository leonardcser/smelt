...
smelt.cmd.register("trust", function()
  ...
  if status == "no_content" then
    smelt.ui.notify("no .smelt/ content under this project; nothing to trust")
    return
  end
  local ok, hash = pcall(smelt.trust.mark)
  if not ok then
    smelt.ui.notify_error("trust: " .. tostring(hash))
    return
  end
  smelt.ui.notify("project trusted: " .. hash:sub(1, 12) .. "; restart to load .smelt/")
end, {
