...
local function install_stable_async(tag, on_done)
  ...
  smelt.spawn(function()
    ...
    if not target or target == "" then
      smelt.notify.error("/upgrade: unknown target triple; can't pick a release asset")
      return done(false, "unknown target")
    end
    local exe, err = smelt.os.exe_path()
    if not exe then
      smelt.notify.error("/upgrade: " .. tostring(err))
    ...
    smelt.notify("✓ upgraded to " .. tag .. " — restart smelt to use it")
    done(true)
  end)
end
...
local function install_unstable_async(sha, on_done)
  ...
  smelt.spawn(function()
    ...
    if not r then
      smelt.notify.error("/upgrade: failed to spawn cargo")
...
smelt.cmd.register("upgrade", function(args)
  ...
  if args:match("%-%-check") or args == "check" then
    run_check(true)
    smelt.notify("checking for upgrades…")
    return
  end
  if args:match("%-%-yes") or args == "yes" or args == "-y" then
    if not latest.has_update then
      smelt.notify("already up to date (" .. (latest.current or "?") .. ")")
      return
    end
    dispatch_install()
    return
