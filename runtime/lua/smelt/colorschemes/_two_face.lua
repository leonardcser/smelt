...
local function build(p)
  ...
  local groups = {
    ...
    Visual              = bg(select),
    Search              = { fg = color(bg0), bg = color(accent), bold = true },
    YankFlash           = bg(mix(p.warn, bg0, p.light and 0.45 or 0.62)),
    ...
    SmeltDiffDeleteInlineBg = bg(del_inline),
  }
  return {
    name = p.module,
    syntax = p.name,
