...
-- 8-wide, 6-row mini-flame keyframes for animated wordmark+fire
-- compositions. All frames share dimensions; cycle through them
-- ~12fps to flicker. Each frame paints the flame on rows 1-5 with
-- row 6 reserved as the gap before the wordmark — varying the upper
-- silhouette (height, lean, spark) keeps the wordmark anchor steady
-- while the flame above feels alive. Same palette as `LOGO_PIXELS`.
M.MINI_FIRE_FRAMES = {
	-- compact, base-heavy
	{
		"........",
	...
	-- leaning left with a detached spark
	{
		"..R.....",
		"..OR.R..",
		".RoOR...",
...
function M.wordmark_size()
	return pixel_grid_size(M.WORDMARK_PIXELS)
end
function M.logo_size()
	return pixel_grid_size(M.LOGO_PIXELS)
end
