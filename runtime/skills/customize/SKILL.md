...
Compile-time build identity and version metadata for plugins.
...
- `smelt.session.list` :: `fun(opts: table?): table`
  List persisted sessions other than the current one from the derived catalog.
...
- `smelt.plugin` (Host) :: `fun(name: string): smelt.Plugin`
...
List bundled colorschemes, or apply, read, and override the active one.
