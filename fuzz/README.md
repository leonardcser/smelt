...
Anything after the target name is forwarded to libFuzzer. The two flags above
the `--` shim live on the xtask itself:
...
2. Don't poll - the harness fires a notification the moment any background
   ...
   panic; the artifact is under `fuzz/artifacts/<target>/`.
...
Then either fix the bug and commit a regression seed, or - if you want to keep
poking - replay the shrunk scenario directly:
...
`fuzz/seeds/<target>/regression/`. CI replays them on every PR; running them
