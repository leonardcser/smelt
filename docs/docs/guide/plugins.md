...
| --- | --- | --- |
| `compact` | yes | Owns `/compact` and post-turn auto-compaction (uses `smelt.engine.ask_with_trim`, drives the prompt top-bar working indicator via `smelt.spinner.busy`) |
