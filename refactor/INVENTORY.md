...
| `dialog.rs`           | 1890 | Multi-panel dialog with PanelWidget           | deleted      | P1.0      | pending | Replaced by Overlay + LayoutTree + N Windows; deferred from P0                                      |
...
| `layout.rs`           | 832  | `LayoutTree` (Vbox/Hbox/Leaf(WinId)) + Constraint(7) + Chrome + Placement(legacy) + natural_size | restructured | P1.b/P1.c | partial | P1.b ✅: Vbox/Hbox/Leaf(WinId) + Item tuple + 7 Constraint variants + Chrome (gap/border/title); resolve_layout returns HashMap<WinId, Rect>. P1.c partial: `Anchor` enum + `LayoutTree::natural_size((cap))` ✅. P1.c remaining: SeparatorStyle on Chrome; Placement deletion (paired with last float migration). |
