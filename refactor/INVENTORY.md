...
| `lib.rs`              | 2400 | `Ui` facade                                   | restructured | P1.f      | pending | Rewritten with splits/overlays/focus/hit_test API; P1.c partial: overlay storage + per-frame resolution + active_modal + overlay_hit_test + canonical `focus()` / `set_focus(win)` / `focus_history` + `focused_overlay` + `focused_window`/`focused_window_mut` + `overlay_close` pops focus_history + unified `hit_test` returning `HitTarget` + modal-aware `focus_next` / `focus_prev` + `Ui::render` walks resolved overlays via `Compositor::render_with` + modal-Esc-dismiss built-in + `overlay_focus` field + `overlay_for_leaf` + `set_focus` accepts overlay leaves + leaf-keymap dispatch (C.3, C.4, C.4-tails 1–6, C.5.0–C.5.4). |
