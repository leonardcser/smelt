...
| `component.rs`        | 100  | `Component` trait + `WidgetEvent`             | deleted      | P1.d      | partial | C.9c.1 dropped dead `WidgetEvent::{Submit, Cancel, SelectDefault}` variants. Trait + remaining variants retire when Picker / Cmdline / StatusBar / Notification / BufferView / WindowView migrate to overlay leaves in P1.d (tracked via C.9c.2). |
| `compositor.rs`       | 663  | Layer manager + z-order + dispatch            | restructured | P1.f      | pending | Folds into `Ui` facade with splits/overlays.                                                          |
...
| `lib.rs`              | 2600 | `Ui` facade                                   | restructured | P1.f      | partial | Splits/overlays/focus/hit_test API. P1.c overlay surface (storage + resolution + paint + focus + leaf-event routing + modal dismiss) operational; C.9c.2 dropped `notification_open` / `notification_mut`. Full facade rewrite still ahead. |
