...
| `dialog.rs`           | 1685 | Multi-panel dialog with PanelWidget           | deleted      | P1.0      | pending | Replaced by Overlay + LayoutTree + N Windows; deferred from P0.                                       |
...
| `lua/api/widgets.rs`                      | 633  | Lua widget creation API                                     | restructured        | P3.b      | partial | `smelt.win.buf` + `smelt.win.set_focus` + `smelt.buf.get_line` Lua primitives landed. P3.b splits into ui.rs / win.rs / buf.rs. |
...
| `lua/ui_ops.rs`                           | 924  | Lua UI ops (overlay open/close)                             | restructured        | P3.b      | partial | Every Lua dialog routes through `open_dialog_via_overlay`. C.9b dropped the `kind = "input"` `collapse_when_empty` widget fallback + the unreachable legacy `dialog_open` call site + `parse_dialog_placement`; `OverlayPlacement::DockBottom` parses `placement = "dock_bottom"` + `placement_height` into `Anchor::ScreenBottom { above_rows: 1 }` + `Percentage(height_pct)`. List leaves fire `SelectionChanged` from `move_cursor`. Picker still uses `FloatConfig` / `Placement` until its own migration. P3.b splits into ui.rs / win.rs. |
