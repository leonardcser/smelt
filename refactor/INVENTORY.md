...
| `lib.rs`              | 1713 | `Ui` facade                                   | restructured | P1.f      | pending | Rewritten with splits/overlays/focus/hit_test API; P1.c partial: overlay storage + `resolve_overlays(cursor)` + `active_modal` + `overlay_hit_test` (modal-aware, Chrome/Window split) landed (C.3, C.4, C.4-tail₂). |
