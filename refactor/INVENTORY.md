...
| `app/mod.rs`                              | 2099 | God-struct App + event loop                                 | restructured        | P2.a/P2.b | partial | 8 subsystem fields now hang off `App.core: Core`; final TuiApp / HeadlessApp split is P2.a.12b. |
...
| `app/core.rs`                             | 48   | `Core` aggregate of headless-safe subsystems (`config` / `session` / `confirms` / `clipboard` / `timers` / `cells` / `lua` / `engine`); reached as `App.core.<field>` | kept                | P2.a.12a  | landed  | `tools: ToolRuntime` slot stays vacant pending P5.a's trait shape (a.10 deferred); P2.a.12b wraps `Core` directly inside `TuiApp` / `HeadlessApp`. |
