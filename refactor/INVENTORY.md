...
| `lib.rs`              | 2619 | `Ui` facade + `UiHost` trait                  | restructured | P1.f/P2.b.2/P2.b.3/P2.b.4c.3 | partial | Splits / overlays / focus / hit_test / capture / cursor_shape API operational; `dispatch_event(ui::Event) -> ui::Status` consumes the ui-owned terminal-event enum and now owns the scrollbar drag gesture (Down latches Scrollbar capture + snaps `Ui::wins[owner].scroll_top`; Drag re-snaps; Up releases); `hit_test` returns `HitTarget::Scrollbar` for splits leaves with painted bars; `Ui::win_buf{,_id,_mut}(WinId)` resolves WellKnown WinIds; `UiHost` trait + `Ui` impl shipped in P2.b.2 (no `Host` supertrait, kept independent so `ui` cannot reference tui-defined types). Full facade rewrite still ahead (F.6 tail). |
