...
| `app/cells.rs`                            | 960  | `Cells` reactive primitive (typed name → value + per-`TypeId` Lua projector + glob subs + queue-then-drain + `build_with_builtins`) | kept                | P2.a.4    | landed  | All built-in cells declared and projector-backed; `turn_complete` / `turn_error` publish call sites move with EngineBridge in a.11. |
...
| `lua/confirm_ops.rs`                      | 233  | Lua confirm bindings                                        | moved-to-lua        | P4.d      | partial | Routes through `app.confirms.*`. Migration to `dialogs/confirm.lua` stays P4.d. |
