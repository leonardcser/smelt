...
| `overlay.rs`          | 390  | `Overlay` + `OverlayId` + `OverlayHitTarget` + `HitTarget` + `Anchor` resolver | added        | P1.c      | partial | Types + resolution + hit-test landed (incl. `Anchor::ScreenBottom` for dock-bottom positioning). C.9c deletes `FloatConfig` / `Placement`. |
...
| `lua/api/widgets.rs`                      | 571  | Lua widget creation API                                     | restructured        | P3.b      | partial | `smelt.win.buf` + `smelt.buf.get_line` Lua primitives landed. P3.b splits into ui.rs / win.rs / buf.rs. |
...
| `lua/ui_ops.rs`                           | 715  | Lua UI ops (overlay open/close)                             | restructured        | P3.b      | partial | Content / list / options / input panel kinds route through `open_dialog_via_overlay` with Buffer-backed leaves. `OptionList` `has_shortcut || has_multi` legacy branch deleted in C.9a (no consumers). Buffer-backed leaves now honor `spec.collapse_when_empty` via `Constraint::Length(0)`. `kind = "input"` `collapse_when_empty` widget fallback (last legacy escape) flips in C.9b. P3.b splits into ui.rs / win.rs. |
...
| `dialog.lua`                      | 257 | Dialog handle factory + framework     | kept         | P4.a  | pending | Primary dialog abstraction.                                                                          |
