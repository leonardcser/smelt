...
| `layout.rs`           | 606  | `LayoutTree` (Vbox/Hbox/Leaf(WinId)) + Constraint(7) + Chrome + Placement(legacy) + Anchor(corner) | restructured | P1.b/P1.c | partial | P1.b ✅: Vbox/Hbox/Leaf(WinId) + Item tuple + 7 Constraint variants + Chrome (gap/border/title); resolve_layout returns HashMap<WinId, Rect>. P1.c remaining: Placement → richer Anchor enum (Screen/Cursor/Win); SeparatorStyle on Chrome. |
| `lib.rs`              | 1419 | `Ui` facade                                   | restructured | P1.f      | pending | Rewritten with splits/overlays/focus/hit_test API                                                   |
...
| `overlay.rs`          | 268  | `Overlay` + `OverlayId` + `Anchor` resolver   | added        | P1.c      | partial | C.1-C.3 ✅: types + resolve_anchor + Ui storage. C.4+ pending: compositor integration + consumer migration |
