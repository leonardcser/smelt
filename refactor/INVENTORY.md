...
| `lua/ui_ops.rs`                           | 617  | Lua UI ops (overlay open/close)                             | restructured        | P3.b      | partial | C.6 ✅: content-only Lua dialogs route to `open_dialog_via_overlay` → `Overlay { vbox(border+title, vbox(leaves)), ScreenCenter, modal }`. C.7.1 ✅: `kind = "list"` panel kind + `LeafShape::{ Content, List }` discriminator + `configure_list_leaf` (cursor-line-highlight on, j/k/Up/Down/PageDown/PageUp/Home/End/g navigation callbacks, Enter → `Submit { Selection { abs_row } }` via `CallbackResult::Event`). Pure-list and content-only dialogs route to overlay; mixed (options/input + list) still go through `dialog_open` until C.7.2+ migrates `OptionList`/`TextInput`. P3.b splits into ui.rs / win.rs. |
