...
| `app/cells.rs`                            | 477  | `Cells` reactive primitive (typed name → value + Lua subscribers + glob subs + queue-then-drain) | kept                | P2.a.4    | partial | a.4c migrates built-in cells (stateful + event-shaped) + adds `SubscriberKind::Rust(...)`. |
