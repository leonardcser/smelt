...
| `lua/api/mod.rs`                          | 303  | Lua API module organization                                 | restructured        | P3.b      | partial | Orchestrates per-namespace `cmd` / `keymap` / `task` / `tools` / `statusline` / `timer` / `cell` / `au` / `spawn` registers alongside the legacy `state` / `widgets` umbrellas; `dispatch.rs` retired. State + widgets are the last two umbrellas. |
...
| `lua/api/cell.rs`                         | 215  | `smelt.cell` reactive registry bindings + `CellHandle` userdata | added           | P3.b      | landed  | Flat API + `smelt.cell(name)` userdata both reach `Cells`.                                                                   |
| `lua/api/cmd.rs`                          | 80   | `smelt.cmd.{register,run,list}` — slash command registry     | added               | P3.b      | landed  | Writes to `LuaShared.commands`.                                                                                              |
...
| `lua/api/spawn.rs`                        | 30   | `smelt.spawn` — fire-and-forget Lua coroutine                | added               | P3.b      | landed  | Routes through `LuaShared.tasks`.                                                                                            |
| `lua/api/statusline.rs`                   | 60   | `smelt.statusline.{register,unregister}` segment sources     | added               | P3.b      | landed  | Writes to `LuaShared.statusline_sources`.                                                                                    |
| `lua/api/task.rs`                         | 45   | `smelt.task.{alloc,resume}` external-id resolution surface   | added               | P3.b      | landed  | Drives coroutine yield primitives in `_bootstrap.lua`.                                                                       |
...
| `lua/api/timer.rs`                        | 63   | `smelt.timer.{set,every,cancel}` + `smelt.defer` alias       | added               | P3.b      | landed  | Schedules through `host.timers()` (Host-tier).                                                                               |
