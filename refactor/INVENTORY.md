...
| `compositor.rs`       | 610  | Layer manager + z-order + dispatch            | restructured | P1.f      | pending | Folds into `Ui` facade with splits/overlays. `clear_focus()` added in C.4-tail₄ for the overlay-close pop path.       |
...
| `layout.rs`           | 1014 | `LayoutTree` (Vbox/Hbox/Leaf(WinId)) + Constraint(7) + Chrome (gap/border/title/separator) + Placement(legacy) + natural_size + contains_leaf + leaves_in_order | restructured | P1.b/P1.c | partial | P1.b ✅: Vbox/Hbox/Leaf(WinId) + Item tuple + 7 Constraint variants. P1.c partial: `Anchor` enum + `LayoutTree::natural_size((cap))` + `SeparatorStyle` on Chrome + `LayoutTree::contains_leaf(WinId)` + `LayoutTree::leaves_in_order()` ✅. P1.c remaining: Placement deletion (paired with last float migration). |
| `lib.rs`              | 2170 | `Ui` facade                                   | restructured | P1.f      | pending | Rewritten with splits/overlays/focus/hit_test API; P1.c partial: overlay storage + per-frame resolution + active_modal + overlay_hit_test + canonical `focus()` / `set_focus(win)` / `focus_history` + `focused_overlay` + `focused_window`/`focused_window_mut` + `overlay_close` pops focus_history + unified `hit_test` returning `HitTarget` + modal-aware `focus_next` / `focus_prev` (C.3, C.4, C.4-tails 1–6). |
