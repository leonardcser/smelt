...
| `layout.rs`           | 1098 | `LayoutTree` Vbox/Hbox/Leaf(WinId) + Constraint(7) + Chrome + Anchor(5) + natural_size + paint_chrome | restructured | P1.b/P1.c | partial | P1.b target shape landed. C.9a added `Anchor::ScreenBottom { above_rows }` for dock-bottom overlays. C.9c deletes the legacy `Placement` enum once every dialog flips. |
| `lib.rs`              | 2452 | `Ui` facade                                   | restructured | P1.f      | partial | Splits/overlays/focus/hit_test API. P1.c overlay surface (storage + resolution + paint + focus + leaf-event routing + modal dismiss + focused-leaf hardware cursor) operational; C.9c.2 dropped `notification_open` / `notification_mut`; C.9c.3 dropped `cmdline_open` / `cmdline_mut` / `cmdline` and added `focused_overlay_cursor`. Full facade rewrite still ahead. |
| `overlay.rs`          | 455  | `Overlay` + `OverlayId` + `OverlayHitTarget` + `HitTarget` + `Anchor` resolver | added        | P1.c      | partial | Types + resolution + hit-test landed (incl. `Anchor::ScreenBottom` for dock-bottom positioning + `Anchor::Win { row_offset, col_offset }` from C.9c.2). `Overlay::blocks_agent` (C.9b) gates engine-event drain when set. C.9c.5 deletes `FloatConfig` / `Placement`. |
...
| `window.rs`           | 1170 | Window viewport                               | restructured | P1.d      | partial | Read-only `render(buf, slice, ctx)` + cursor-row highlight + extmark highlight painting landed. Selection / scrollbar / gutters wait for P1.d alongside `BufferView` deletion. |
