...
| `app/mod.rs`                              | 1574 | God-struct App + event loop                                 | restructured        | P2.a/P2.b | partial | `App.core: Core`; headless / subagent flows live on `HeadlessApp` now. P2.a.12b2 renames `App` → `TuiApp`. |
...
| `app/core.rs`                             | 83   | `Core` aggregate of headless-safe subsystems (`config` / `session` / `confirms` / `clipboard` / `timers` / `cells` / `lua` / `engine`); reached as `App.core.<field>` | kept                | P2.a.12a  | landed  | `Core::new(config, engine)` constructed by both `App::new` (TUI) and `HeadlessApp::new`; `tools: ToolRuntime` slot stays vacant pending P5.a's trait shape; P2.a.12b2 renames `App` → `TuiApp`. |
...
| `app/headless_app.rs`                     | 528  | `HeadlessApp { core, sink, next_turn_id }` frontend: one-shot `run_oneshot` + persistent `run_subagent` | kept                | P2.a.12b1 | landed  | First non-TUI frontend over `Core`. P2.a.12b2's App → TuiApp rename keeps this file untouched. |
