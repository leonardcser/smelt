...
| `app/mod.rs`                              | 1870 | God-struct App + event loop                                 | restructured        | P2.a/P2.b | partial | P1.d.3 opens `TRANSCRIPT_WIN` over `transcript_display_buf`. P1.d.4 drops the dead `set_layout` + `drag_on_layer`. P1.d.5b adds App-level `clipboard: ui::Clipboard`. P1.f.3 seeds splits tree via `ui.set_layout` each frame. P1.f.4 drops `drag_on_scrollbar` + `ScrollbarDragTarget` (Ui::capture owns it). Final carve into AppConfig/WellKnown/Session/Confirms/Clipboard/Timers/Cells/LuaRuntime/ToolRuntime/EngineBridge stays P2.a/P2.b. |
...
| `content/prompt_data.rs`                  | 971  | Prompt content (formatting, completion)                     | restructured        | P2/P4     | partial | P1.d.2b: `compute_prompt` populates the unified prompt buffer + drives `wins[PROMPT_WIN]` cursor / viewport. P1.f.3: `fixed = row_offset + 1` (status owns its own splits leaf). Final widget-recipe migration is P4. |
