...
| `compositor.rs`       | 614  | Layer manager + z-order + dispatch            | restructured | P1.f      | pending | Folds into `Ui` facade with splits/overlays                                                         |
...
| `layout.rs`           | 940  | `LayoutTree` (Vbox/Hbox/Leaf(WinId)) + Constraint(7) + Chrome (gap/border/title/separator) + Placement(legacy) + natural_size | restructured | P1.b/P1.c | partial | P1.b ✅: Vbox/Hbox/Leaf(WinId) + Item tuple + 7 Constraint variants. P1.c partial: `Anchor` enum + `LayoutTree::natural_size((cap))` + `SeparatorStyle` on Chrome (None/Solid/Dashed; `with_separator` auto-inflates gap 0→1) ✅. P1.c remaining: Placement deletion (paired with last float migration). |
| `lib.rs`              | 1843 | `Ui` facade                                   | restructured | P1.f      | pending | Rewritten with splits/overlays/focus/hit_test API; P1.c partial: overlay storage + `resolve_overlays(cursor)` + `active_modal` + `overlay_hit_test` + canonical `focus()` / `set_focus(win)` / `focus_history` (C.3, C.4, C.4-tail₂, C.4-tail₃). |
