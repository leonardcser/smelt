...
| `window.rs`           | 1170 | Window viewport                               | restructured | P1.d      | partial | Read-only `render(buf, slice, ctx)` + cursor-row highlight + extmark highlight painting landed. C.9c.5 inlined `WinConfig` to `SplitConfig` directly and dropped `is_float` / `is_split` / `zindex` / `FloatConfig`. Selection / scrollbar / gutters wait for P1.d alongside `BufferView` deletion. |
