...
| `lib.rs`              | 2619 | `Ui` facade + `UiHost` trait                  | restructured | P1.f/P2.b.2/P2.b.3 | partial | Splits / overlays / focus / hit_test / capture / cursor_shape API operational; `dispatch_event(ui::Event) -> ui::Status` consumes the ui-owned terminal-event enum; `Ui::win_buf{,_id,_mut}(WinId)` resolves WellKnown WinIds; `UiHost` trait + `Ui` impl shipped in P2.b.2 (no `Host` supertrait, kept independent so `ui` cannot reference tui-defined types). Full facade rewrite still ahead (F.6 tail). |
...
| `app/mouse.rs`                            | 691  | Mouse routing + drag + scrollbar                            | restructured        | P1/P2     | partial | Viewport reads via `Ui::win(id)?.viewport`; click-count via `Ui::record_click`; scrollbar drag + per-pane dispatch fold via UiHost across P2.b.4c.3–c.6. |
