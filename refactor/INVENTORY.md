...
| `window.rs`           | 1170 | Window viewport                               | restructured | P1.d      | partial | C.5.1 ✅: minimal `render(buf, slice, ctx)` (read-only viewer scope — no extmark layered paint, no scrollbar, no gutters yet). Single interactive unit + `handle(event, ctx, host) -> Status` shape comes in P1.d alongside `BufferView` deletion + the rest of the render surface.                |
...
| `lua/ui_ops.rs`                           | 510  | Lua UI ops (overlay open/close)                             | restructured        | P3.b      | partial | C.6 ✅: content-only Lua dialogs route to `open_dialog_via_overlay` → `Overlay { vbox(border+title, vbox(leaves)), ScreenCenter, modal }`; mixed dialogs (options/input/list) still go through `dialog_open` until widgets get Buffer-backed rewrites in C.7+. P3.b splits into ui.rs / win.rs. |
