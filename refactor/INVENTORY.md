...
| `window.rs`           | 1170 | Window viewport                               | restructured | P1.d      | partial | C.5.1 ✅: minimal `render(buf, slice, ctx)` (read-only viewer scope — no extmark layered paint, no scrollbar, no gutters yet). C.7.0 ✅: `cursor_line_highlight: bool` opt-in field; focused leaves with the opt-in set paint cursor row with `theme.get("CursorLine")` background. Single interactive unit + `handle(event, ctx, host) -> Status` shape comes in P1.d alongside `BufferView` deletion + the rest of the render surface.                |
