...
| `lib.rs`              | 2619 | `Ui` facade + `UiHost` trait                  | restructured | P1.f/P2.b.2/P2.b.3/P2.b.4c.3/P2.b.4c.5a/P2.b.4c.5b | partial | Splits / overlays / focus / hit_test / capture / cursor_shape API operational; `dispatch_event(ui::Event) -> ui::Status` consumes the ui-owned terminal-event enum and now owns the scrollbar drag gesture; `Ui::resolve_split_mouse(MouseEvent) -> Option<(WinId, u8)>` latches `HitTarget::Window` capture on Down + records click count, returns the captured win on Drag/Up + clears capture on Up; `hit_test` returns `HitTarget::Scrollbar` for splits leaves with painted bars; `UiHost` trait + `Ui` impl shipped in P2.b.2 + grew `viewport_for(WinId)` / `rows_for(WinId)` / `breaks_for(WinId)` per-pane accessors in P2.b.4c.5b (no `Host` supertrait). Full facade rewrite still ahead (F.6 tail). |
