...
| `lua/api/notebook.rs`                     | 81   | `smelt.notebook.render(buf, args)` — `edit_notebook` preview | added               | P3.b      | landed  | Reuses `ConfirmPreview::from_tool` + `render_into_buffer`; carries its own shallow Lua → JSON arg helper.                    |
...
| `lua/api/process.rs`                      | 86   | `smelt.process.{list,kill,read_output,spawn_bg}` over the engine ProcessRegistry | added | P3.b | landed | Background processes share registry with `bash run_in_background=true`.                                                       |
