...
| `lib.rs`              | 1559 | `Ui` facade                                   | restructured | P1.f      | pending | Rewritten with splits/overlays/focus/hit_test API; P1.c partial: overlay storage + `resolve_overlays(cursor)` per-frame helper landed (C.3 + C.4). |
...
| `overlay.rs`          | 365  | `Overlay` + `OverlayId` + `Anchor` resolver   | added        | P1.c      | partial | C.1-C.4 ✅: types + `resolve_anchor` + Ui storage + per-frame `Ui::resolve_overlays` (drives `LayoutTree::natural_size` then `resolve_anchor`). C.5+ pending: first float migration to Overlay; consumer migration; deletion of `FloatConfig` / `Placement`. |
