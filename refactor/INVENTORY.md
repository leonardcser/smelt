...
| `component.rs`        | 100  | `Component` trait + `WidgetEvent`             | deleted      | P1.d      | partial | C.9c.1 dropped `WidgetEvent::{Submit, Cancel, SelectDefault}`; C.9c.3 dropped `WidgetEvent::{SubmitText, TextChanged}` (cmdline widget retired). Trait + `WidgetEvent::{Dismiss, Select}` retire when Picker / StatusBar / BufferView / WindowView migrate to overlay leaves in P1.d. |
