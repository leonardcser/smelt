...
| --------------------- | ---- | --------------------------------------------- | ------------ | --------- | ------- | --------------------------------------------------------------------------------------------------- |
| `buffer.rs`           | 1521 | Lines, namespaced extmarks, marks, virtual text | restructured | P1.a    | partial | Extmark + namespace + `YankSubst` + `yank_text_for_range` + `wrap_at` cache + `BufferParser` rename / `on_attach` hook landed; `attach(spec)` parser-hook system + `BufferFormatter` deletion deferred to P1.a-tail (gated on transcript migration) |
...
| `callback.rs`         | 367  | Window callbacks + WinEvent routing           | kept         | P2        | pending | Drain pipeline for Lua callbacks; pattern preserved. C.7.0.5: `CallbackResult::Event(WinEvent, Payload)` lets Rust callbacks fire WinEvents without `lua_invoke` access (drops `Eq, PartialEq` from CallbackResult — Payload doesn't impl Eq, no consumers compared with `==`). |
...
| `component.rs`        | 116  | `Component` trait + `WidgetEvent`             | deleted      | P1.0      | pending | Window is the only interactive unit; deferred from P0                                               |
| `compositor.rs`       | 663  | Layer manager + z-order + dispatch            | restructured | P1.f      | pending | Folds into `Ui` facade with splits/overlays. `clear_focus()` added in C.4-tail₄ for the overlay-close pop path. `render_with(theme, w, after_layers)` added in C.5.0 — overlay paint hook between layer paint and flush.       |
| `dialog.rs`           | 1685 | Multi-panel dialog with PanelWidget           | deleted      | P1.0      | pending | Replaced by Overlay + LayoutTree + N Windows; deferred from P0. P1.c-tail: deduped `SeparatorStyle` against `layout::SeparatorStyle`; the local enum + `Option<…>` wrapper deleted. |
...
| `layout.rs`           | 1098 | `LayoutTree` (Vbox/Hbox/Leaf(WinId)) + Constraint(7) + Chrome (gap/border/title/separator) + Placement(legacy) + natural_size + contains_leaf + leaves_in_order + paint_chrome | restructured | P1.b/P1.c | partial | P1.b ✅: Vbox/Hbox/Leaf(WinId) + Item tuple + 7 Constraint variants. P1.c partial: `Anchor` enum + `LayoutTree::natural_size((cap))` + `SeparatorStyle` on Chrome + `LayoutTree::contains_leaf(WinId)` + `LayoutTree::leaves_in_order()` + `paint_chrome(grid, area, chrome, theme)` ✅. P1.c remaining: Placement deletion (paired with last float migration). |
| `lib.rs`              | 2600 | `Ui` facade                                   | restructured | P1.f      | pending | Rewritten with splits/overlays/focus/hit_test API; P1.c partial: overlay storage + per-frame resolution + active_modal + overlay_hit_test + canonical `focus()` / `set_focus(win)` / `focus_history` + `focused_overlay` + `focused_window`/`focused_window_mut` + `overlay_close` pops focus_history + unified `hit_test` returning `HitTarget` + modal-aware `focus_next` / `focus_prev` + `Ui::render` walks resolved overlays via `Compositor::render_with` + per-frame `ensure_rendered_at(leaf_width)` pre-pass + modal-Esc-dismiss built-in + `overlay_focus` field + `overlay_for_leaf` + `set_focus` accepts overlay leaves + leaf-keymap dispatch + `win_close` routes to `overlay_close` on overlay leaves + `overlay_root_for_leaf` + `dispatch_event` redirects leaf events to overlay root + modal-Esc fires Dismiss once on root (C.3, C.4, C.4-tails 1–6, C.5.0–C.5.4, C.6, C.7.5). |
...
| `overlay.rs`          | 390  | `Overlay` + `OverlayId` + `OverlayHitTarget` + `HitTarget` + `Anchor` resolver | added        | P1.c      | partial | C.1-C.4 + tails ✅: types + `resolve_anchor` + Ui storage + per-frame `resolve_overlays` + modal/hit-test layer (`OverlayHitTarget` per-overlay + `HitTarget` global). C.5+ pending: first float migration; deletion of `FloatConfig` / `Placement`. |
...
| `theme.rs`            | 120  | Highlight group registry (nvim-style)         | kept         | P1.0      | landed  | Replaces host `crate::theme::*` flat module; plumbed through DrawContext. C.7.0: `CursorLine` group added (drives Window cursor-row highlight).                            |
...
| `window.rs`           | 1170 | Window viewport                               | restructured | P1.d      | partial | C.5.1 ✅: minimal `render(buf, slice, ctx)` (read-only viewer scope — no scrollbar, no gutters yet). C.7.0 ✅: `cursor_line_highlight: bool` opt-in field; focused leaves with the opt-in set paint cursor row with `theme.get("CursorLine")` background. C.7.4 ✅: layered highlight extmark painting via `merge_span_style(base, span)` — `Some` colors override, bool attrs OR. Selection paint + scrollbar + gutters still wait for P1.d alongside `BufferView` deletion. |
...
| `app/dialogs/text_modal.rs`               | 70   | Read-only text modal (`/stats`, `/cost`)                    | moved-to-lua        | P4        | partial | C.5.3 + C.5.4 ✅: rewritten as `Overlay { vbox(border+title, hbox(leaf)), ScreenCenter, modal }`; Esc via Ui built-in; `q` + `Ctrl+C` via leaf `win_set_keymap` callbacks calling `overlay_for_leaf` → `overlay_close`. Full parity vs old DialogConfig path. P4 moves it to Lua. |
...
| `lua/api/widgets.rs`                      | 571  | Lua widget creation API                                     | restructured        | P3.b      | partial | C.7.6 ✅: `smelt.win.buf(win) -> buf_id` (Window's backing Buffer) + `smelt.buf.get_line(buf, line_idx)` (1-based; `nil` out of range). C.8 ✅: `smelt.ui.dialog._open` returns `(win_id, named_inputs)` multi-value where `named_inputs[name] = leaf_win_id` for every overlay-routed `kind = "input"` panel. P3.b splits into ui.rs / win.rs / buf.rs. |
...
| `lua/ui_ops.rs`                           | 715  | Lua UI ops (overlay open/close)                             | restructured        | P3.b      | partial | C.6 ✅: content-only Lua dialogs route to `open_dialog_via_overlay` → `Overlay { vbox(border+title, vbox(leaves)), ScreenCenter, modal }`. C.7.1 ✅: `kind = "list"` panel kind + `LeafShape::{ Content, List { initial_cursor } }` discriminator + `configure_list_leaf` (cursor-line-highlight on, j/k/Up/Down/PageDown/PageUp/Home/End/g navigation callbacks, Enter → `Submit { Selection { abs_row } }` via `CallbackResult::Event`). C.7.2 ✅: `kind = "options"` panels build a Buffer-backed list leaf via `make_options_buffer` + route through overlay (legacy widget path retained behind `has_shortcut || has_multi` guard). C.8 ✅: `LeafShape::Input { name }` variant; `kind = "input"` panels build a Buffer-backed input leaf via `make_input_buffer` (dim placeholder extmark on row 0) + `configure_input_leaf` (Backspace/Left/Right/Home/End/Enter keymaps + printable-char fallback under `KeyModifiers::NONE | SHIFT`); insert/backspace fire `WinEvent::TextChanged`, Enter fires `Submit { Text { content } }`; `_open` returns `(win_id, named_inputs)` so dialog.lua can read each input leaf's buffer text via `smelt.win.buf` + `smelt.buf.get_line`. `confirm.lua`'s `collapse_when_empty` input row stays on legacy widget path until C.9. P3.b splits into ui.rs / win.rs. |
...
| `dialog.lua`                      | 257 | Dialog handle factory + framework     | kept         | P4.a  | pending | Primary dialog abstraction. C.8 ✅: `_open` consumed as `(win_id, named_inputs)`; `collect_inputs(name)` falls back to `smelt.win.buf(leaf)` + `smelt.buf.get_line(buf, 1)` when the panel doesn't carry a `text` field (overlay-routed input panels). |
