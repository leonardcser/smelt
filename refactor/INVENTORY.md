...
| `lib.rs`              | 2464 | `Ui` facade                                   | restructured | P1.f      | partial | Splits / overlays / focus / hit_test / capture / cursor_shape API operational; `dispatch_event(Event)` consumes terminal events; `Ui::win_buf{,_id,_mut}(WinId)` resolves WellKnown WinIds. Full facade rewrite still ahead (F.6 tail). |
...
| `app/mod.rs`                              | 2099 | God-struct App + event loop                                 | restructured        | P2.a/P2.b | partial | Final carve into Clipboard / Timers / LuaRuntime / ToolRuntime / EngineBridge subsystems + Core / TuiApp / HeadlessApp split is P2.a.3b+/P2.b. |
| `app/agent.rs`                            | 1436 | Multi-agent lifecycle, message relay                        | restructured        | P5.b/P5.c | partial | Lua-side multi-agent registry migration stays P5.b/P5.c. |
...
| `app/cells.rs`                            | 789  | `Cells` reactive primitive (typed name → value + per-`TypeId` Lua projector + glob subs + queue-then-drain + `build_with_builtins`) | kept                | P2.a.4    | partial | a.4c.3 publishes event-shaped cells (`history`, `turn_complete`, `turn_error`, `confirm_*`, `session_*`); some entries gated on EngineBridge a.11. |
...
| `app/confirms.rs`                         | 62   | `Confirms` subsystem (pending tool-approval requests)       | kept                | P2.a.3    | partial | oneshot::Sender swap + `is_clear()` gate ride P2.a.3b alongside `EngineBridge`. |
...
| `lua/api/dispatch.rs`                     | 710  | Lua FFI dispatch                                            | restructured        | P3.b      | partial | Hosts `smelt.timer.*` + `smelt.cell.*` + `smelt.au.*` + `smelt.on` registrations; per-namespace P3.b split still pending. |
