...
| `app/cells.rs`                            | 695  | `Cells` reactive primitive (typed name → value + per-`TypeId` Lua projector + glob subs + queue-then-drain + `build_with_builtins`) | kept                | P2.a.4    | partial | a.4c.2 wires timer-driven cells (`now`, `spinner_frame`, `tokens_used`); a.4c.3 publishes event-shaped cells. |
