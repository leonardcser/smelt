...
| `layout.rs`           | 1098 | `LayoutTree` Vbox/Hbox/Leaf(WinId) + Constraint(7) + Chrome + natural_size + paint_chrome | restructured | P1.b/P1.c | partial | P1.b target shape landed. P1.c remaining: `Placement` deletion (paired with last float migration). |
...
| `overlay.rs`          | 390  | `Overlay` + `OverlayId` + `OverlayHitTarget` + `HitTarget` + `Anchor` resolver | added        | P1.c      | partial | Types + resolution + hit-test landed. C.9 deletes `FloatConfig` / `Placement`. |
...
| `lua/ui_ops.rs`                           | 715  | Lua UI ops (overlay open/close)                             | restructured        | P3.b      | partial | Content / list / options / input panel kinds route through `open_dialog_via_overlay` with Buffer-backed leaves. Legacy `OptionList` retained behind `has_shortcut || has_multi`; `confirm.lua` `collapse_when_empty` row stays on legacy path until C.9. P3.b splits into ui.rs / win.rs. |
