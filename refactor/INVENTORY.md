...
| --------------------- | ---- | --------------------------------------------- | ------------ | --------- | ------- | --------------------------------------------------------------------------------------------------- |
| `buffer.rs`           | 1521 | Lines, namespaced extmarks, marks, virtual text | restructured | P1.a    | partial | Extmark + namespace + `YankSubst` + `wrap_at` + `BufferParser` rename landed. `attach(spec)` + `BufferFormatter` deletion deferred to P1.a-tail. |
...
| `component.rs`        | 100  | `Component` trait + `WidgetEvent`             | deleted      | P1.d.4    | partial | C.9c.1 dropped `WidgetEvent::{Submit, Cancel, SelectDefault}`; C.9c.3 dropped `WidgetEvent::{SubmitText, TextChanged}` (cmdline widget retired); P1.d.1 retired the `StatusBar` Component impl by moving the status line onto a Buffer-backed Window painted via `Ui::render`. Trait + `WidgetEvent::{Dismiss, Select}` retire in P1.d.4 when the remaining `BufferView` / `WindowView` consumers (transcript / prompt) migrate. |
| `compositor.rs`       | 664  | Layer manager + z-order + dispatch            | restructured | P1.f      | partial | C.9c.3: `render_with` closure now returns `Option<(u16, u16)>` so the focused overlay leaf gets a hardware cursor (overlay > layer). Full fold into `Ui` facade still ahead.                                                          |
...
| `status_bar.rs`       | 174  | Statusline widget (deleted)                   | deleted      | P1.d.1    | landed  | C.9c.5 deferred Component deletion to P1.d; P1.d.1 retired the `StatusBar` widget itself by migrating the status line onto a Buffer-backed Window painted via `Ui::render`. The Lua statusline migration is a separate concern (P4.c → `widgets/statusline.lua` + Cells). |
...
| `content/window_view.rs`                  | 352  | Window-specific content view                                | merged              | P1.d      | partial | P1.d.2b retired `set_rows` / `set_cursor` / `WindowContent::Rows` / `WindowRow` / `StyledSegment` (the prompt's row-rendering path). Transcript still uses `sync_from_buffer` / `add_highlight` / `set_viewport` / `set_soft_cursor` until P1.d.3 / P1.d.4 finish the migration. |
