...
| `lua/api/mod.rs`                          | 295  | Lua API module organization                                 | restructured        | P3.b      | partial | Declares per-namespace `timer` / `cell` / `au` files alongside the legacy `dispatch` / `state` / `widgets` umbrellas; remaining `dispatch` namespaces (cmd / keymap / task / tools / statusline / spawn) split next. |
...
| `lua/api/dispatch.rs`                     | 252  | Lua FFI dispatch (task / tools / statusline / spawn)        | restructured        | P3.b      | partial | `cmd` / `keymap` / `timer` / `cell` / `au` carved into sibling files; remaining four namespaces still multiplexed here.       |
