...
| `layout.rs`           | 973  | `LayoutTree` (Vbox/Hbox/Leaf(WinId)) + Constraint(7) + Chrome (gap/border/title/separator) + Placement(legacy) + natural_size + contains_leaf | restructured | P1.b/P1.c | partial | P1.b ✅: Vbox/Hbox/Leaf(WinId) + Item tuple + 7 Constraint variants. P1.c partial: `Anchor` enum + `LayoutTree::natural_size((cap))` + `SeparatorStyle` on Chrome + `LayoutTree::contains_leaf(WinId)` ✅. P1.c remaining: Placement deletion (paired with last float migration). |
| `lib.rs`              | 2048 | `Ui` facade                                   | restructured | P1.f      | pending | Rewritten with splits/overlays/focus/hit_test API; P1.c partial: overlay storage + per-frame resolution + active_modal + overlay_hit_test + canonical `focus()` / `set_focus(win)` / `focus_history` + `focused_overlay` + `focused_window`/`focused_window_mut` + `overlay_close` pops focus_history + unified `hit_test` returning `HitTarget` (C.3, C.4, C.4-tails 1–5). |
