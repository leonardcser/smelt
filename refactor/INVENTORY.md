...
| `app/dialogs/text_modal.rs`               | 41   | Read-only text modal (`/stats`, `/cost`)                    | moved-to-lua        | P4        | partial | C.5.3 ✅: rewritten as `Overlay { vbox(border+title, hbox(leaf)), ScreenCenter, modal }`; Esc dismiss via Ui built-in. `dialog_open` path no longer used. P4 still moves it to Lua. Lost `q`/`Ctrl+C` dismiss until leaf-level overlay event routing (P1.d/P1.e). |
