...
| `lib.rs`              | 2230 | `Ui` facade                                   | restructured | P1.f      | partial | Splits / overlays / focus / hit_test / capture API. P1.c overlay surface (storage / resolution / paint / focus / leaf events / modal dismiss / focused-leaf cursor) operational. P1.d.1/2a/2b/3/4 land painted-split paint path + retire compositor `Component` machinery + `KeyResult::{Action,Capture}`. P1.f.1 collapsed dual focus slots into `Ui::focus`; P1.f.2 `overlays: Vec<(OverlayId, Overlay)>`; P1.f.3 `splits: LayoutTree` + `set_layout(tree)` / `resolve_splits()` / `split_rect()`; P1.f.4 `capture: Option<HitTarget>` + `set_capture` / `clear_capture` (auto-clear on `set_layout` / `overlay_close`). Full facade rewrite still ahead (F.5/F.6). |
...
| `window.rs`           | 1170 | Window viewport                               | restructured | P1.d      | partial | Read-only `render(buf, slice, ctx)` + cursor-row + extmark highlight painting landed. P1.d.2a/b add `cursor_kind` (`Hardware` / `Block`) + `viewport` + scrollbar paint. P1.d.3 transcript selection lands as extmarks. P1.d.5b drops `kill_ring`. P1.d.5c/e/f.1 hoist all per-Window vim state inline (`vim_state: VimWindowState`, `selection_anchor`, `curswant`). P1.d.5f.2a swaps `Option<Vim>` for `vim_enabled: bool`. Remaining decomposition waits for P1.d.5d/f.2d. |
...
| `app/render_loop.rs`                      | 447  | Per-frame render                                            | kept                | P2.e      | partial | P1.d.2b/3 rewrite `sync_prompt_layer` / `sync_transcript_layer` onto painted-split `Ui::bufs[…]` + drive `wins[…]`'s `cursor_kind` / `viewport`. P1.f.3 builds + publishes the splits tree via `Ui::set_layout` each frame. Single select! loop hookup is still P2.e. |
