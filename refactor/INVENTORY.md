...
| --------------------- | ---- | --------------------------------------------- | ------------ | --------- | ------- | --------------------------------------------------------------------------------------------------- |
| `buffer.rs`           | 1521 | Lines, namespaced extmarks, marks, virtual text | restructured | P1.a    | partial | `attach(spec)` parser-hook system + `BufferFormatter` deletion deferred to P1.a-tail (gated on transcript-pipeline migration). |
...
| `lib.rs`              | 2230 | `Ui` facade                                   | restructured | P1.f      | partial | Splits / overlays / focus / hit_test / capture / cursor_shape API operational; `dispatch_event(Event)` consumes terminal events; `Ui::win_buf{,_id,_mut}(WinId)` resolves WellKnown WinIds. Full facade rewrite still ahead (F.6 tail). |
...
| `window.rs`           | 1170 | Window viewport                               | restructured | P1.d      | partial | `render(buf, slice, ctx)` + cursor + scrollbar paint operational; per-Window vim state inline (`vim_state`, `selection_anchor`, `curswant`); virt-text paints after highlights. Remaining decomposition waits for P1.d.5d/f.2d. |
...
| `app/mod.rs`                              | 2059 | God-struct App + event loop                                 | restructured        | P2.a/P2.b | partial | Final carve into Clipboard / Timers / LuaRuntime / ToolRuntime / EngineBridge subsystems + Core / TuiApp / HeadlessApp split is P2.a.3b+/P2.b. |
| `app/agent.rs`                            | 1427 | Multi-agent lifecycle, message relay                        | restructured        | P5.b/P5.c | partial | Lua-side multi-agent registry migration stays P5.b/P5.c. |
...
| `app/cells.rs`                            | 344  | `Cells` reactive primitive (typed name → value + Lua subscribers + queue-then-drain) | kept                | P2.a.4    | partial | a.4b adds userdata-handle ergonomics + `glob_subscribe` + `smelt.au.*` alias; a.4c migrates built-in cells + adds `SubscriberKind::Rust(...)`. |
...
| `app/confirms.rs`                         | 60   | `Confirms` subsystem (pending tool-approval requests)       | kept                | P2.a.3    | partial | oneshot::Sender swap + `is_clear()` gate ride P2.a.3b alongside `EngineBridge`. |
...
| `app/history.rs`                          | 652  | Rewind/fork/compact state                                   | merged              | P2        | partial | Reads/writes route through `self.session.*`. Final merge into Session subsystem stays P2.a.8+. |
...
| `app/mouse.rs`                            | 691  | Mouse routing + drag + scrollbar                            | restructured        | P1/P2     | partial | Pre-flight via `Ui::dispatch_event`; full hit-test/dispatch fold via Host moves with P2's UiHost trait. |
...
| `lua/api/dispatch.rs`                     | 559  | Lua FFI dispatch                                            | restructured        | P3.b      | partial | Hosts `smelt.timer.*` + `smelt.cell.*` + `smelt.on` registrations; per-namespace P3.b split still pending. |
...
| `lua/confirm_ops.rs`                      | 195  | Lua confirm bindings                                        | moved-to-lua        | P4.d      | partial | Routes through `app.confirms.*`. Migration to `dialogs/confirm.lua` stays P4.d. |
| `lua/mod.rs`                              | 1723 | Lua runtime + autocmd dispatch                              | restructured        | P2.a/P3   | partial | Final LuaRuntime carve + autocmd-into-Cells fold still pending. |
...
| `theme.rs`                                | 355  | Theme bridge: populate_ui_theme + OSC11 detect + PRESETS    | narrowed            | P1.0      | landed  | Bridge fns + presets only; atomic globals collapsed onto `ui::Theme`. |
