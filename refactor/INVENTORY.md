...
| `app/host.rs`                             | 127  | `Host` trait — Ui-agnostic accessors over Core subsystems (`clipboard / cells / timers / lua / engine / session / confirms`); impls for `Core / TuiApp / HeadlessApp` | kept                | P2.b.1    | landed  | `tools()` waits on a.10. Sibling `UiHost` trait (in ui crate) lands in P2.b.2; Window::handle collapse + Lua TLS split ride later P2.b sub-phases. |
