...
| `lib.rs`              | 1990 | `Ui` facade                                   | restructured | P1.f      | pending | Rewritten with splits/overlays/focus/hit_test API; P1.c partial: overlay storage + `resolve_overlays(cursor)` + `active_modal` + `overlay_hit_test` + canonical `focus()` / `set_focus(win)` / `focus_history` + `overlay_focused` + `focused_window`/`focused_window_mut` + `overlay_close` pops focus_history (C.3, C.4, C.4-tails 1–4). |
...
| `overlay.rs`          | 376  | `Overlay` + `OverlayId` + `OverlayHitTarget` + `Anchor` resolver | added        | P1.c      | partial | C.1-C.4 + C.4-tail₂ ✅: types + `resolve_anchor` + Ui storage + per-frame `resolve_overlays` + modal/hit-test layer (`OverlayHitTarget::{Window\|Chrome}`). C.5+ pending: first float migration; deletion of `FloatConfig` / `Placement`. |
