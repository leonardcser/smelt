...
| `lib.rs`              | 2643 | `Ui` facade + `UiHost` trait                  | restructured | P1.f/P2.b.2 | partial | Splits / overlays / focus / hit_test / capture / cursor_shape API operational; `dispatch_event(Event)` consumes terminal events; `Ui::win_buf{,_id,_mut}(WinId)` resolves WellKnown WinIds; `UiHost` trait + `Ui` impl shipped in P2.b.2 (no `Host` supertrait, kept independent so `ui` cannot reference tui-defined types). Full facade rewrite still ahead (F.6 tail). |
