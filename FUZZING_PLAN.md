...
- [x] State invariants — prompt-buffer slice landed: cursor on UTF-8 boundary (`smelt_buffer::text::snap` round-trip) and cursor in `0..=source.len()` checked after every event.
- [ ] Remaining state invariants: viewport bounds, grid width×height, wide-char continuation, undo cap, picker index in range. Step budget per event (default 1000).
