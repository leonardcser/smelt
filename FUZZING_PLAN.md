...
- [ ] Remaining state invariants: viewport bounds, wide-char continuation, undo cap, picker index in range, completer state. Step budget per event (default 1000).
- [ ] Deeper invariants: undo round-trip (apply ops then undo-all → original source), attachment offsets land on UTF-8 boundaries, kill-ring source-range still valid after source mutations. Land after pure `step()` extraction.
