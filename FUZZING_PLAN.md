...
- [x] `fuzz/` crate with `cargo-fuzz`. **Landed**: `fuzz/fuzz_targets/smelt_loop.rs` drives `TestApp` end-to-end against `Arbitrary`-derived `FuzzOp` sequences (printable keys, Ctrl-letters, special keys, virtual-clock ticks, Lua wakeups, resizes). Each event runs under `AllocBudget::DEFAULT` so runaway-allocation regressions fail the corpus entry. Build with `cargo +nightly fuzz run smelt_loop`; the crate is workspace-excluded so default workspace builds stay clean.
- [x] State invariants — generalized over every live `(Buffer, Window)` pair via `TestApp::assert_invariants`: cursor in `0..=source.len()`, cursor on UTF-8 char boundary, same two constraints on `selection_anchor` when set, and a global terminal-size sanity check. The fuzz target calls it after every event.
- [x] Static-state leak invariant inside the fuzz iteration: snapshot `smelt_style::theme::registry_len()` and `smelt_buffer::buffer::namespace_count()` after build, assert end-of-scenario growth stays within `INTERN_SLACK = 64`. Runaway interns surface as the smallest libFuzzer input that grows the registries past slack.
- [x] Input-shape hardening: `FuzzInput` now covers UTF-8 keystrokes (full `u32` codepoint space, surrogates rewritten to `?`), ctrl+shift modifiers, special keys (Enter/Esc/arrows/Home/End/PageUp/Down/Delete), bracketed paste with arbitrary UTF-8, real resizes (width × height clamped to `[1, 400]`), and `AgentMode` selection (Normal/Plan/Apply/Yolo).
- [x] Engine-path fuzzing: `FuzzOp` emits `EngineEvent::{Ready, Text, TextDelta, ThinkingDelta, ToolStarted, ToolOutput, ToolFinished}` plus `ExecOutput` / `ExecDone`, exercising the streaming-text and tool-lifecycle dispatch.
- [x] Harness ring/core hardening: `Osc52Sink` swapped for `NullSink` at build time (no more `\x1b]52;c;...` leaking to test stdout). `SourceEvent::Resize` now carries dimensions explicitly. Engine dispatch consolidated to `TuiApp::dispatch_engine_event`, shared by the production main loop and the harness (eliminates duplicate state-machine branching).
- [ ] Steady-state leak detector — separate target running scenario `N=5` times and asserting peak `current_bytes` growth ≤ 5% scenario-over-scenario. Deferred; per-iteration registry check above catches most leak signals at lower wall-time cost.
- [x] Deeper offset invariants — `TestApp::assert_invariants` now also walks every `(Buffer, UndoEntry)` snapshot pair (undo + redo) and asserts `cpos` is in-bounds and on a UTF-8 boundary against the entry's own buffer text; enforces the undo cap when set; validates `kill_ring.source_range()` is well-formed; checks `PromptState.completer.anchor` is in-bounds + on a boundary in the prompt-edit buffer when a session is active; and asserts the focused window id is alive.
- [x] Agent state-machine activation — `TestApp::start_turn(turn_id)` synthesizes an active `TurnState` so subsequent `SourceEvent::Engine(_)` events route through `handle_engine_event` + `dispatch_control` + tool tracking instead of the idle handler. `FuzzOp::StartTurn` exposes this to the fuzz target. Invariant: pending tool `call_id`s are unique within a turn.
- [x] Scenario serialization — `FuzzInput` derives `Serialize` / `Deserialize` (adjacently-tagged JSON) and doubles as the on-disk `Scenario` format. Shared library `fuzz/src/lib.rs` holds the types, `apply` per-op dispatcher, and `run_scenario` driver; the libFuzzer entry shrinks to one line. Two helper bins: `crash_to_scenario` (artifact bytes → JSON via the same `Arbitrary` decoder libFuzzer uses, so the roundtrip is lossless) and `replay_scenario` (JSON → harness, exits non-zero on any invariant violation). End-to-end roundtrip verified on a real corpus entry.
- [x] Determinism — `TestApp::build` now wipes the shared `$TEST_HOME` tempdir between scenarios. Session / history / state files written by one iteration no longer leak into the next iteration's `load_autoload`. Removed a real source of order-dependent test behavior; surfaced at first by libFuzzer (`MS: 1 InsertByte-` crash that wouldn't reproduce on isolated replay).
- [x] Visual replay — `fuzz/src/bin/play_scenario.rs` drives a scenario through `TestApp` against the user's real terminal (raw mode + alt screen). `TestApp::render` calls `TuiApp::render_normal`, so each step paints the same compositor pipeline production uses; status line at the bottom row shows step counter + next op + controls (`space`/`→` next, `b`/`←` back, `r` reset, `s` state dump, `q`/`Esc` quit). Back / reset rebuild a fresh app and fast-forward via `apply_n`.
- [x] Invariant scoping for line-based buffers — the cursor-vs-source assertion was firing on line-based buffers (cmdline, picker, status bar, list overlays) whose `Buffer::source` stays empty because content lives in `lines`. `assert_invariants` now skips windows whose buffer is empty-source-with-non-empty-lines, and a regression test (`cmdline_typed_payload_does_not_trip_cursor_invariant`) pins the cmdline case.
- [ ] Remaining state invariants: viewport bounds, wide-char continuation, picker selected index in range, attachment-marker count. Step budget per event (default 1000).
- [ ] Undo round-trip — apply ops then undo-all → original source. Useful but requires undo to be exactly invertible across kill-ring side effects; land after pure `step()` extraction.
