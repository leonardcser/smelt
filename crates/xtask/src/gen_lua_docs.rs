...
fn run_inner() -> std::io::Result<()> {
    ...
    //   * functions (smelt.sleep, smelt.dialog.open, smelt.cmd.register_picker, …)
    ...
    let runtime_root = crate::repo_root().join("runtime/lua/smelt");
...
fn push_luacats_doc(out: &mut String, doc: &str) {
...
fn render_stub(
    ...
    if !mod_doc.is_empty() {
        push_luacats_doc(&mut s, mod_doc);
    }
    s.push_str(&format!(
        "--- Classification: {} - {}\n",
    ...
    for f in fns {
        ...
        push_luacats_doc(&mut s, &plain_doc);
...
/// The final value records whether the declaration belongs to the private
/// `__smelt_internal` capability tree.
fn parse_lua_fn_decl(line: &str) -> Option<(String, String, bool)> {
    fn canonical_name(name: &str) -> Option<(String, bool)> {
        if name.starts_with("smelt.") {
            Some((name.to_string(), false))
        } else {
            name.strip_prefix("__smelt_internal.")
                .map(|name| (format!("smelt.{name}"), true))
        }
    }
    ...
    if let Some(rest) = line.strip_prefix("function ") {
        let paren = rest.find('(')?;
        let (name, internal) = canonical_name(rest[..paren].trim())?;
        let close = rest.rfind(')')?;
        if close <= paren {
            return None;
        }
        return Some((name, rest[paren + 1..close].trim().to_string(), internal));
    }
    let eq = line.find('=')?;
    let (name, internal) = canonical_name(line[..eq].trim())?;
    ...
    Some((name, after_fn[1..close].trim().to_string(), internal))
}
...
fn parse_bundled_lua(content: &str, tier: Tier) -> BundledItems {
    ...
    for (lineno0, raw) in content.lines().enumerate() {
        ...
        if let Some((full_name, args, internal)) = parse_lua_fn_decl(raw) {
            ...
            let declared_classification = classification.take();
            let item_classification = if internal || name.starts_with('_') {
                ApiClassification::Internal
            } else {
                declared_classification.unwrap_or_else(|| classification_for_type(&full_name))
            };
...
mod tests {
    ...
    #[test]
    fn luacats_doc_comments_do_not_add_trailing_whitespace() {
        let mut output = String::new();
        push_luacats_doc(&mut output, "first line\n\nlast line");
        assert_eq!(output, "--- first line\n---\n--- last line\n");
    }

    #[test]
    fn private_capability_functions_are_always_internal() {
        let items = parse_bundled_lua(
            r#"
            ---@advanced
            function __smelt_internal.picker.open_prompt(opts) end
            "#,
            Tier::UiHost,
        );
        ...
        assert_eq!(items.fns[0].module, "smelt.picker");
        assert_eq!(items.fns[0].name, "open_prompt");
        assert_eq!(items.fns[0].classification, ApiClassification::Internal);
    }
