...
//!
//! Outputs are derived from `LuaFnMeta` entries pushed by `LuaMod::fn_`
...
fn run_inner() -> std::io::Result<()> {
    ...
    let mod_docs: BTreeMap<&str, (&str, Option<Tier>)> = modules_snapshot()
        .into_iter()
        .map(|m| (m.module, (m.doc, m.tier)))
        .collect();
    ...
    for module in &all_modules {
        ...
        let (mod_doc, declared_tier) = mod_docs.get(module).copied().unwrap_or(("", None));
        ...
        std::fs::write(
            ...
            render_stub(module, fns, mod_doc, tier),
        )?;
        std::fs::write(
            md_dir.join(format!("{file_stem}.md")),
            render_markdown(module, fns, &type_index, mod_doc, tier),
        )?;
    }
    ...
    std::fs::write(
        ...
        render_index(&by_mod, &mod_docs, classes.len(), aliases.len()),
    )?;
    let nav_status = sync_zensical_nav(&zensical, &by_mod)?;
    let skill_status = sync_customize_skill(&root, &by_mod, &mod_docs)?;
...
fn render_stub(module: &str, fns: &[&LuaFnMeta], mod_doc: &str, tier: Tier) -> String {
...
Do not edit by hand. -->\n\n",
    ...
    let mixed_tiers = has_mixed_tiers(fns);
    if mixed_tiers {
        s.push_str("This namespace mixes Host and UiHost functions; each function below lists its exact tier.\n\n");
    }
    if !mod_doc.is_empty() {
        s.push_str(mod_doc);
        s.push_str("\n\n");
...
fn render_index(
    ...
    mod_docs: &BTreeMap<&str, (&str, Option<Tier>)>,
    n_classes: usize,
    n_aliases: usize,
) -> String {
    let mut s = String::new();
...
fn sync_customize_skill(
    ...
    let api_body = render_skill_api_index(by_mod, mod_docs);
...
fn render_skill_api_index(
    ...
    for label in ["Host", "UiHost", "Mixed"] {
        ...
        for (module, fns) in rows {
            s.push_str(&format!("#### `{module}`\n\n"));
            if let Some((doc, _)) = mod_docs.get(*module) {
                if !doc.is_empty() {
...
fn render_skill_settings() -> String {
    ...
    for decl in SETTINGS {
        ...
        let doc = decl.doc.trim();
        let doc_md = if doc.is_empty() { "" } else { doc };
