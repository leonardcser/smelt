...
fn render_stub(module: &str, fns: &[&LuaFnMeta], mod_doc: &str, _tier: Tier) -> String {
...
for a working config.\n\n",
    ...
    for (module, fns) in by_mod {
        let tier = mod_docs
            ...
            .or_else(|| fns.first().map(|f| f.tier))
            .unwrap_or(Tier::Host);
        by_tier.entry(tier.label()).or_default().push((module, fns));
    }
...
fn parse_bundled_lua(content: &str) -> BundledItems {
