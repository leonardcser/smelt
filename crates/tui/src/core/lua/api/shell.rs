...
//! `crate::core::permissions::split_shell_commands` for AST-level
...
fn check_interactive(command: &str) -> Option<&'static str> {
    let cmds = crate::core::permissions::split_shell_commands(command);
    for subcmd in &cmds {
...
fn check_shell_background_operator(command: &str) -> Option<String> {
    let has = crate::core::permissions::split_shell_commands_with_ops(command)
        .iter()
...
pub(super) fn register(lua: &Lua, smelt: &mlua::Table) -> LuaResult<()> {
    ...
    shell_tbl.set(
        ...
        lua.create_function(|_, command: String| {
            Ok(crate::core::permissions::split_shell_commands(&command))
        })?,
    )?;
    shell_tbl.set(
        "split_with_ops",
        lua.create_function(|lua, command: String| {
            let parts = crate::core::permissions::split_shell_commands_with_ops(&command);
    ...
    shell_tbl.set(
        ...
        lua.create_function(|_, pattern: String| {
            Ok(crate::core::permissions::DEFAULT_BASH_ALLOW.contains(&pattern.as_str()))
        })?,
    )?;
