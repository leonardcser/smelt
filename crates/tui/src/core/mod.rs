    ...
    pub async fn run(
        ...
        'main: loop {
            ...
            loop {
                ...
                // dispatch so `engine_client::handle_event` can
                // borrow its fields while we still hold `&mut self`.
                let action = if let Some(mut ag) = self.agent.take() {
                    let ctrl = engine_client::handle_event(self, ev, ag.turn_id, &mut ag.pending);
                    ...
                    engine_client::handle_idle_event(self, ev);
                    LoopAction::Continue
                };
                match action {
                    LoopAction::Continue => {}
                ...
                && self.agent.is_some()
            {
                    ...
                    && self.agent.is_some()
                {
                    ...
                    let pending_ref: &[crate::core::PendingTool] =
            ...
            tokio::select! {
                ...
                Some(ev) = self.core.engine.recv() => {
                    if let Some(mut ag) = self.agent.take() {
                        let ctrl =
                            engine_client::handle_event(self, ev, ag.turn_id, &mut ag.pending);
                        let action = self.dispatch_control(
                            ctrl,
                            &ag.pending,
                            &mut pending_dialogs,
